...
public class CombinedFluidHandler implements IFluidHandler {
    ...
    public FluidStack drain(int maxDrain, FluidAction action) {
        ...
        for (IFluidHandler handler : this.handlers) {
            ...
            FluidStack drained = handler.drain(amount, FluidAction.SIMULATE);
            if (!drained.isEmpty() && (stack.isEmpty() || stack.equals(drained))) {
                stack = handler.drain(maxDrain, action).copy();
                amount -= stack.getAmount();
            }
        }
        if (stack.isEmpty() || amount == maxDrain) {
            return FluidStack.EMPTY;
        } else {
            stack.setAmount(maxDrain - amount);
            return stack;
        }
    }
}
