...
public class GashatItemRenderer extends BlockEntityWithoutLevelRenderer {
    ...
    public void renderByItem(ItemStack pStack, ItemTransforms.TransformType pTransformType, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        ...
        if (pStack.getItem() instanceof MaterialItem) {
            switch (pTransformType) {
                case FIXED, GUI -> Renders.with(pPoseStack, () -> {
                    ...
                    pPoseStack.translate(-0.5, -0.5, -0.5);
                    GameOrbRenderer.renderHalo(pPoseStack, buffer, pTransformType, pPackedLight, pPackedOverlay, 0xC0E0E0FF, false);
                });
            }
        }

        Renders.with(pPoseStack, () ->
                this.renderModel(pPoseStack, buffer, pTransformType, pPackedLight, pPackedOverlay, pStack));
        ...
        if (this.particle) {
            ...
            PARTICLES.removeIf(particle -> particle.render(pStack, pTransformType, pPoseStack, pBuffer, pPackedLight, pPackedOverlay));
        }
    }

    public void renderModel(PoseStack poseStack, VertexConsumer consumer, ItemTransforms.TransformType transformType, int light, int overlay, ItemStack stack) {
        ...
        if (this.rotation) {
            ...
            poseStack.mulPose(Vector3f.ZP.rotationDegrees(cos * 8));
            poseStack.mulPose(Vector3f.XP.rotationDegrees(cos * 2));
            poseStack.mulPose(Vector3f.YP.rotationDegrees(cos * 2));
        }

        BakedModel model = ForgeHooksClient.handleCameraTransforms(poseStack, Minecraft.getInstance().getModelManager().getModel(this.model), transformType, transformType == ItemTransforms.TransformType.FIRST_PERSON_LEFT_HAND || transformType == ItemTransforms.TransformType.THIRD_PERSON_LEFT_HAND);
        poseStack.translate(-0.5, -0.5, -0.5);
    ...
    public static class Particle {
        ...
        public boolean render(ItemStack stack, ItemTransforms.TransformType transformType, PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay) {
            if (Util.getMillis() - this.made >= this.age) {
                ...
                Renders.with(poseStack, () -> {
                    ...
                    poseStack.mulPose(Vector3f.ZP.rotationDegrees(cos * 20));
