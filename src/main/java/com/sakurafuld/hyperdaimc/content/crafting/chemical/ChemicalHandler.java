...
public class ChemicalHandler {
    ...
    @SubscribeEvent
    public static void mutation(LivingEvent.LivingUpdateEvent event) {
                ...
                if (mutation >= 0) {
                    ...
                    require(LogicalSide.SERVER).run(() -> {
                        if (mutation > 80 && !MutekiHandler.muteki(zombie)) {
                            Player player = zombie.getLevel().getNearestPlayer(zombie, 32);
                            double yRot;
                            if (player != null) {
                            ...
                            list.forEach(at -> {
                                if (zombie.getLevel().getBlockState(at).getMaterial().isReplaceable()) {
                                    zombie.getLevel().setBlockAndUpdate(at, state);
                                } else {
                                    Block.popResource(zombie.getLevel(), at, state.getBlock().asItem().getDefaultInstance());
                                }
                                zombie.getLevel().levelEvent(LevelEvent.PARTICLES_DESTROY_BLOCK, at, Block.getId(state));
