...
public class DeskMenu extends AbstractContainerMenu {
    ...
    public DeskMenu(int id, Inventory inventory, FriendlyByteBuf buf) {
        this(id, inventory, inventory.player.getLevel().getBlockEntity(buf.readBlockPos(), HyperBlockEntities.DESK.get()).orElseThrow());
    }

    public DeskMenu(int id, Inventory inventory, DeskBlockEntity tile) {
        super(HyperMenus.DESK.get(), id);
        this.access = ContainerLevelAccess.create(inventory.player.getLevel(), tile.getBlockPos());
        this.data = tile.data;
