...
public class DeskShapelessRecipe implements IDeskRecipe {
    ...
    public static class Serializer extends ForgeRegistryEntry<RecipeSerializer<?>> implements RecipeSerializer<DeskShapelessRecipe> {
        @Override
        public DeskShapelessRecipe fromJson(ResourceLocation recipeLoc, JsonObject recipeJson, ICondition.IContext context) {
            ...
            if (GsonHelper.isArrayNode(recipeJson, "values")) {
                ...
                for (JsonElement element : GsonHelper.getAsJsonArray(recipeJson, "values")) {
                    ...
                    if (value.startsWith("#")) {
                        items.addAll(context.getTag(ItemTags.create(new ResourceLocation(value.substring(1)))).getValues().stream().map(Holder::value).toList());
                    } else {
                        items.add(ForgeRegistries.ITEMS.getValue(new ResourceLocation(value)));
                    }
                }
                ...
                if (GsonHelper.isArrayNode(recipeJson, "exclusion")) {
                    for (JsonElement element : GsonHelper.getAsJsonArray(recipeJson, "exclusion")) {
                        ...
                        if (exclusion.startsWith("#")) {
                            ...
                            values.remove(ForgeRegistries.ITEMS.getValue(new ResourceLocation(exclusion)));
                        }
                    }
                }
