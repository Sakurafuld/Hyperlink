...
public class ChronicleHandler {
    ...
    public static void logIn(PlayerEvent.PlayerLoggedInEvent event) {
        if (event.getEntity() instanceof ServerPlayer player) {
            ChronicleSavedData.get(player.level()).sync2Client(player);
        }
    }
    ...
    public static void pause(PlayerInteractEvent.LeftClickBlock event) {
        if (isPaused(event.getLevel(), event.getPos(), event.getEntity())) {
...
//    }
    ...
    public static void pause(BlockEvent.EntityPlaceEvent event) {
        if (event.getLevel() instanceof Level level && isPaused(level, event.getPos(), event.getEntity())) {
    ...
    public static void pause(BlockEvent.FarmlandTrampleEvent event) {
        if (event.getLevel() instanceof Level level && isPaused(level, event.getPos(), null)) {
            event.setCanceled(true);
        }
    }
...
//    }
    ...
    public static void pause(LivingDestroyBlockEvent event) {
        if (isPaused(event.getEntity().level(), event.getPos(), event.getEntity())) {
            event.setCanceled(true);
        }
    }
