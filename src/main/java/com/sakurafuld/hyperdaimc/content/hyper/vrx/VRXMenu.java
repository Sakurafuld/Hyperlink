...
public class VRXMenu extends AbstractContainerMenu {
    ...
    public VRXMenu(int id, Inventory inventory, Pair<Pair<BlockPos, Integer>, Direction> pair) {
        ...
        CapabilityProvider<?> provider = this.execute(player.getLevel(), block -> block, entity -> entity);
    ...
    public void closedByKey(ServerPlayer player) {
        ...
        this.execute(player.getLevel(), block -> {
            ...
            if (different) {
                VRXHandler.playSound(player.getLevel(), Vec3.atCenterOf(block.getBlockPos()), !map.isEmpty() || !nulls.isEmpty());
            }
            ...
            if (different) {
                VRXHandler.playSound(player.getLevel(), entity.position(), !map.isEmpty() || !nulls.isEmpty());
            }
        }));
