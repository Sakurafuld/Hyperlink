    ...
    public static void novel(InputEvent.ClickInputEvent event){
        ...
        if(event.isAttack() && mc.player.getMainHandItem().is(ModItems.NOVEL.get())){
            ...
            if(mc.player.isShiftKeyDown()){
                ...
                entities.stream().min(Comparator.comparingDouble(entity-> entity.position().distanceToSqr(eye)))
                        .ifPresent(entity -> {
                ...
                List<Entity> entities = rayTraceEntities(mc.player, eye, eye.add(view.x() * reach, view.y() * reach, view.z() * reach), mc.player.getBoundingBox().expandTowards(view.scale(reach)).inflate(1), 0.5f);
