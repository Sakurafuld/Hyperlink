...
public class VRXMenu extends AbstractContainerMenu {
    ...
    public VRXMenu(int pContainerId, Inventory inventory, FriendlyByteBuf buf) {
        this(pContainerId, inventory, parse(buf));
    }
    ...
    public boolean stillValid(Player pPlayer) {
        ...
        return !this.available.isEmpty() && this.execute(pPlayer.level(), Objects::nonNull, Objects::nonNull);
    }
        ...
        if (!list.isEmpty()) {
            ...
            this.execute(player.level(), block -> {
                if (block != null) {
                    ...
                    data.sync2Client(player.level()::dimension);
                ...
                if (entity != null) {
                    entity.getCapability(VRXCapability.CAPABILITY).ifPresent(vrx -> {
                        ...
                        PacketHandler.INSTANCE.send(PacketDistributor.DIMENSION.with(player.level()::dimension), new ClientboundVRXSyncCapability(entity.getId(), vrx.serializeNBT()));
    ...
    public void updateTooltip(Player player) {
        if (!this.available.isEmpty()) {
            ...
            if (player instanceof ServerPlayer serverPlayer) {
                ...
                PacketHandler.INSTANCE.send(PacketDistributor.PLAYER.with(() -> serverPlayer), new ClientboundVRXSetTooltip(this.containerId, list));
            }
        }
