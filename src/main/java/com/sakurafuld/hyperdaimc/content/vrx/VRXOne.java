...
import com.sakurafuld.hyperdaimc.api.CombinedFluidHandler;
import com.sakurafuld.hyperdaimc.api.ContainedFluidHandlerItem;
...
public abstract class VRXOne {
    ...
    @OnlyIn(Dist.CLIENT)
    public abstract void renderTooltip(VRXScreen screen, PoseStack poseStack, int x, int y);
    //.
    ...
    public static class Item extends VRXOne {
        ...
        public Object prepareInsert(CapabilityProvider<?> provider, Direction face, List<VRXOne> previous) {
            LazyOptional<IItemHandler> capability = provider.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, face);
            if (capability.isPresent()) {
                IItemHandler handler = capability.orElseThrow(IllegalStateException::new);
                long count = -previous.stream()
                        .filter(one -> one instanceof Item item && this.stack.sameItem(item.stack))
        ...
        public void insert(CapabilityProvider<?> provider, @Nullable Direction face, Object prepared) {
            if (prepared instanceof Long count && count < this.getQuantity()) {
                provider.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, face).ifPresent(handler -> {
        ...
        public static List<VRXOne> collect(CapabilityProvider<?> provider, Direction face) {
            ...
            provider.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, face).ifPresent(handler -> {
                for (int index = 0; index < handler.getSlots(); index++) {
                    ItemStack stack = handler.getStackInSlot(index);
                    if (!stack.isEmpty()) {
                        list.add(new VRXOne.Item(stack.copy()));
                    }
                }
            });
        ...
        public void render(PoseStack poseStack, int x, int y) {
            ...
            mc.getItemRenderer().renderAndDecorateItem(mc.player, this.stack, x, y, 0);
            mc.getItemRenderer().renderGuiItemDecorations(mc.font, this.stack, x, y, "");
            if (this.getQuantity() != 1) {
                poseStack.translate(0, 0, 200);
                Renders.slotScaledString(poseStack, String.valueOf(this.getQuantity()), x, y);
            }
        }
        ...
        public void renderTooltip(VRXScreen screen, PoseStack poseStack, int x, int y) {
            if (screen.getMenu().getCarried().isEmpty()) {
                screen.renderItemTooltip(poseStack, x, y, this.stack);
            }
        }
    ...
    public static class Fluid extends VRXOne {
        ...
        public Object prepareInsert(CapabilityProvider<?> provider, Direction face, List<VRXOne> previous) {
            ...
            LazyOptional<IFluidHandler> capability = provider.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, face);
            if (capability.isPresent()) {
                ...
                if (optional.isPresent()) {
                    ...
                    if (items instanceof IItemHandlerModifiable modifiable) {
                        for (int index = 0; index < items.getSlots(); index++) {
                            LazyOptional<IFluidHandlerItem> fluids = items.getStackInSlot(index).getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY);
                            if (fluids.isPresent()) {
        ...
        public static List<VRXOne> collect(CapabilityProvider<?> provider, Direction face) {
            List<VRXOne> list = Lists.newArrayList();
            provider.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, face).ifPresent(handler -> {
                for (int index = 0; index < handler.getTanks(); index++) {
                    FluidStack stack = handler.getFluidInTank(index);
                    if (!stack.isEmpty()) {
                        list.add(new VRXOne.Fluid(stack.copy()));
                    }
                }
            });
        ...
        @OnlyIn(Dist.CLIENT)
        public void render(PoseStack poseStack, int x, int y) {
            Renders.fluidStack(poseStack, this.stack, x, y);
        ...
        public void renderTooltip(VRXScreen screen, PoseStack poseStack, int x, int y) {
            if (screen.getMenu().getCarried().isEmpty()) {
                ...
                tooltip.add(this.stack.getFluid().getAttributes().getDisplayName(this.stack));
    ...
    public static class Energy extends VRXOne {
        ...
        @Override
        public Object prepareInsert(CapabilityProvider<?> provider, Direction face, List<VRXOne> previous) {
            LazyOptional<IEnergyStorage> capability = provider.getCapability(CapabilityEnergy.ENERGY, face);
            if (capability.isPresent() && capability.orElseThrow(IllegalStateException::new).canReceive()) {
                ...
                if (optional.isPresent()) {
                    IItemHandler items = optional.orElseThrow(IllegalStateException::new);
                    for (int index = 0; index < items.getSlots(); index++) {
                        capability = items.getStackInSlot(index).getCapability(CapabilityEnergy.ENERGY);
                        if (capability.isPresent() && capability.orElseThrow(IllegalStateException::new).canReceive()) {
        ...
        @Override
        public void insert(CapabilityProvider<?> provider, @Nullable Direction face, Object prepared) {
            provider.getCapability(CapabilityEnergy.ENERGY, face).ifPresent(handler -> {
                int count = 0;
            ...
            provider.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, face).ifPresent(items -> {
                for (int index = 0; index < items.getSlots(); index++) {
                    items.getStackInSlot(index).getCapability(CapabilityEnergy.ENERGY).ifPresent(handler -> {
                        int count = 0;
                        while (count <= 1024 && handler.canReceive() && handler.receiveEnergy(this.quantity, true) > 0) {
                            handler.receiveEnergy(Integer.MAX_VALUE, false);
                            count++;
        ...
        public static boolean check(CapabilityProvider<?> provider, Direction face) {
            return provider.getCapability(CapabilityEnergy.ENERGY, face).isPresent();
        }
        ...
        public void renderTooltip(VRXScreen screen, PoseStack poseStack, int x, int y) {
            if (screen.getMenu().getCarried().isEmpty()) {
                List<Component> tooltip = Lists.newArrayList();
                tooltip.add(new TranslatableComponent("tooltip.hyperdaimc.vrx_energy"));
                tooltip.add(new TranslatableComponent("tooltip.hyperdaimc.vrx_energy_description").withStyle(ChatFormatting.GRAY));
                tooltip.add(new TextComponent("Minecraft Forge").withStyle(ChatFormatting.BLUE, ChatFormatting.ITALIC));
                screen.renderTooltip(poseStack, tooltip, Optional.empty(), x, y);
            }
        }
