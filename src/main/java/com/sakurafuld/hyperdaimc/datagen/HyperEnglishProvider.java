...
public class HyperEnglishProvider extends LanguageProvider {
    ...
    public HyperEnglishProvider(DataGenerator gen) {
        super(gen, HYPERDAIMC, "en_us");
    }
    ...
    @Nullable
    private String specialize(IForgeRegistryEntry<?> entry) {
    ...
    private String defaultName(IForgeRegistryEntry<?> entry) {
        ...
        String special = specialize(entry);
        if (special != null) {
            return special;
        }

        return Arrays.stream(entry.getRegistryName().getPath().split("_")).map(s -> {
