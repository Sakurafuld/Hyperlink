...
public class Renders {
    ...
    public static void fluidStack(PoseStack poseStack, FluidStack stack, float x, float y) {
        ...
        TextureAtlasSprite sprite = Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(stack.getRawFluid().getAttributes().getStillTexture(stack));
        int color = stack.getRawFluid().getAttributes().getColor(stack);
        int red = (color >> 16) & 0xFF;
        int green = (color >> 8) & 0xFF;
        int blue = color & 0xFF;
    ...
    public static void slotScaledString(PoseStack poseStack, String text, double x, double y) {
        ...
        Renders.with(poseStack, () -> {
            ...
            font.drawInBatch(text, 0, 0, 0xFFFFFF, true, poseStack.last().pose(), bufferSource, false, 0, LightTexture.FULL_BRIGHT);
