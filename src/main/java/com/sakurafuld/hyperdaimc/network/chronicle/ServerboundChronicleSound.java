...
public class ServerboundChronicleSound {
    ...
    public void handle(Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (this.restart) {
                ctx.get().getSender().level().playSound(null, this.position.x(), this.position.y(), this.position.z(), HyperSounds.CHRONICLE_RESTART.get(), SoundSource.PLAYERS, 1, 1);
            } else {
                ctx.get().getSender().level().playSound(null, this.position.x(), this.position.y(), this.position.z(), HyperSounds.CHRONICLE_PAUSE.get(), SoundSource.PLAYERS, 1, 1);
